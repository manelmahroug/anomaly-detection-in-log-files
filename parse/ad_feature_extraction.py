...
def get_clusters(csv_log_file, split_test_train=False):
    ...
    while k < 100 or smallest_cluster_size > max_smallest_cluster_size:
        #kmeans = KMeans(n_clusters=k, n_init=10, algorithm='elkan').fit(tvec_weights_sample)
        kmeans = MiniBatchKMeans(n_clusters=k, n_init=10, batch_size=10000).fit(tvec_weights_sample)
...
def redo_clusters():
    ...
    for file in cluster_files:
        ...
        while k < num_clusters + 5:
            #kmeans = KMeans(n_clusters=k, n_init=10, algorithm='elkan').fit(tvec_weights_sample)
            kmeans = MiniBatchKMeans(n_clusters=k, n_init=10, batch_size=10000).fit(X_tfidf)
